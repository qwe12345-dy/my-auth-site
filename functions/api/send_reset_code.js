import { generateCode, jsonResponse, sendEmailJS, getChinaTime } from '../_utils.js';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!exist) {
      return jsonResponse({ success: false, message: '该邮箱未注册' }, 400);
    }
    const code = generateCode();
    const chinaTime = getChinaTime();
    const expires = new Date(Date.now() + 10 * 60 * 1000).toISOString();
    await env.DB.prepare('INSERT INTO email_codes (email, code, expires_at, created_at) VALUES (?, ?, ?, ?)').bind(email, code, expires, chinaTime).run();
